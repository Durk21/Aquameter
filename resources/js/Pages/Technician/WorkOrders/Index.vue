...
<template>
    ...
    <AppShell>
        ...
        <div class="max-w-3xl mx-auto px-4 md:px-6 py-6 space-y-8">
            ...
            <section>
                <h3 class="font-semibold text-ocean-900 dark:text-white mb-3">My Jobs</h3>
                <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-ocean-100 dark:border-white/5 shadow-soft overflow-hidden">
                    <div v-if="myJobs.length === 0" class="p-6 text-ocean-600 dark:text-neutral-400 text-sm">
                    ...
                    <div v-else class="divide-y divide-ocean-100 dark:divide-white/5">
                        <div v-for="wo in myJobs" :key="wo.id" class="p-4">
                            <div class="flex items-start justify-between gap-3 mb-2">
                                <div>
                                    <p class="font-medium text-ocean-900 dark:text-white">{{ wo.type_label }} · {{ wo.account_number }}</p>
                                    <p class="text-xs text-ocean-500 dark:text-neutral-400 mt-0.5">{{ wo.customer_name }} · {{ wo.dispatch_zone }}</p>
                                    <p v-if="wo.phone" class="text-xs text-ocean-700 dark:text-neutral-300 font-medium mt-0.5">{{ wo.phone }}</p>
                                </div>
                                <span
                            ...
                            <textarea
                                ...
                                class="block w-full text-sm border-ocean-300 dark:border-white/10 dark:bg-neutral-800 dark:text-white focus:border-ocean-500 focus:ring-ocean-500 rounded-lg shadow-sm mb-2"
                            ></textarea>
                            <div class="mb-2">
                                <p class="text-xs font-medium text-ocean-500 dark:text-neutral-400 mb-1">Repair evidence (optional)</p>
                                <PhotoUploadInput
            ...
            <section>
                <h3 class="font-semibold text-ocean-900 dark:text-white mb-3">
                    In Your Zone
                    <span v-if="myZone" class="text-ocean-500 dark:text-neutral-400 font-normal text-sm">({{ myZone }})</span>
                </h3>
                <p v-if="!myZone" class="text-sm text-ocean-500 dark:text-neutral-400 mb-3">
                ...
                <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-ocean-100 dark:border-white/5 shadow-soft overflow-hidden">
                    <div v-if="inZone.length === 0" class="p-6 text-ocean-600 dark:text-neutral-400 text-sm">
            ...
            <section>
                <h3 class="font-semibold text-ocean-900 dark:text-white mb-3">Other Zones</h3>
                <p class="text-sm text-ocean-500 dark:text-neutral-400 mb-3">
                    Not in your zone, but claimable if no one else picks them up.
                </p>
                <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-ocean-100 dark:border-white/5 shadow-soft overflow-hidden">
                    <div v-if="otherZones.length === 0" class="p-6 text-ocean-600 dark:text-neutral-400 text-sm">
